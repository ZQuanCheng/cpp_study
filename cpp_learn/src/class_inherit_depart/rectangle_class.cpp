...
Rectangle::Rectangle():Shape(), ab(), rec(0)                                          // :Shape() 显式调用基类_Shape的缺省构造函数   :ab() 显式调用子对象ab的缺省构造函数
{
    ...
    cout << "Rectangle缺省构造函数" << endl;
}
// _Rectangle带部分缺省参数值的构造函数                                                       // classname::classname():[直接基类初始化表], [子对象初始化表], [数据成员初始化表]
Rectangle::Rectangle(int a, int b, int c, int d, int e, int ptr_value, unsigned int n):Shape(a, b), ab(c, d), rec(e) // :Shape(a, b) 显式调用基类_Shape的带部分缺省参数值的构造函数  :ab(c, d) 显式调用子对象ab的带部分缺省参数值的构造函数
{
    ...
    *ptr = ptr_value;      // 可以直接ptr = new int(ptr_value)
    ...
    cout << "Rectangle带部分缺省参数值的构造函数" << endl;
}
// _Rectangle拷贝构造函数                                                                   // classname::classname():[直接基类初始化表], [子对象初始化表], [数据成员初始化表]
Rectangle::Rectangle(const Rectangle &obj):Shape(obj), ab(obj.ab), rec(obj.rec)     // :Shape(obj) 显式调用基类_Shape的拷贝构造函数    :ab(obj.ab) 显式调用子对象ab的带部分缺省参数值的构造函数
{
...
void Rectangle::print_virtual()  // .h头文件的类中声明已有virtual void print(); 这里不需要virtual修饰
{
    Shape::print_virtual();   
    ab.print();
    ...
    if(ptr != NULL)   
    {
        cout << setw(20) << " Rectangle class: " << setw(20) << " _rec = " << rec << setw(20) <<  " *ptr = " << *ptr;
    }
    else   // 如果指针ptr = NULL, 就不能直接打印cout<< ptr; 否则运行时会报错Segmentation fault (core dumped)
    {
        cout << setw(20) << " Rectangle class: " << setw(20) << " _rec = " << rec;
    }
...
void Rectangle::print()  // .h头文件的类中声明已有virtual void print(); 这里不需要virtual修饰
{
    Shape::print();  
    ab.print(); 
